...
import au.edu.usyd.reviewer.client.core.gwt.DynamicTableDataProvider.RowDataAcceptor;
...
public class DynamicTableWidget extends Composite {
  ...
  private class NavBar extends Composite implements ClickHandler {
    ...
    public NavBar() {
      ...
      bar.setCellVerticalAlignment(status, HasAlignment.ALIGN_TOP);
  ...
  private class RowDataAcceptorImpl implements RowDataAcceptor {
	  ...
	  public void accept(int startRow, Object[][] data) {
	      ...
	      for (; srcRowIndex < srcRowCount; ++srcRowIndex, ++destRowIndex) {
	        Object[] srcRowData = data[srcRowIndex];
	        for (int srcColIndex = 0; srcColIndex < destColCount; ++srcColIndex) {
	          Object obj = srcRowData[srcColIndex];
	          if ( obj instanceof String) {
	        	  grid.setText(destRowIndex, srcColIndex, (String) obj);
	          }
	          else if (obj instanceof Button ){
	        	  grid.setWidget(destRowIndex, srcColIndex, (Button) obj);
	          }        	   
	        }
      }
  ...
  private final DynamicTableDataProvider provider;

  private int startRow = 0;

  public DynamicTableWidget(DynamicTableDataProvider provider, String[] columns,
